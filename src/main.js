...
router.beforeEach(async (to, from, next) => {
  ...
  if (window.localStorage.getItem("tokenStr")) {
    ...
    if (window.localStorage.getItem("userData")) {
      let local = JSON.parse(window.localStorage.getItem("userData"));
      store.commit("initUserData", local);
      console.log("从内存里面拿东西了");
    }
    //判断用户信息是否存在
    else if (!window.localStorage.getItem("userData")) {
      console.log("从数据库拿东西了");
      ...
      store.commit("initUserData", res.data);
      next();
    }
    next();
  } else {
